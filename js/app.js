...
const validarCorreoLogin = () => {
	if (
		correoLoginInput.value.length === 0 ||
...
const nombre_completoRegisterInput = document.querySelector(
	"#nombre_completoRegister"
);
...
const validarDocumentoRegister = () => {
	if (
		documentoRegisterInput.value.length === 0 ||
		documentoRegisterInput.value === "" ||
		documentoRegisterInput.value === null ||
		/^\s+$/.test(documentoRegisterInput.value)
	) {
		Swal.fire({
			icon: "error",
			title: "Error...",
...
const validarNombreRegister = () => {
	if (
		nombre_completoRegisterInput.value.length === 0 ||
		nombre_completoRegisterInput.value === "" ||
		nombre_completoRegisterInput.value === null ||
		/^\s+$/.test(nombre_completoRegisterInput.value)
	) {
		Swal.fire({
			icon: "error",
			title: "Error...",
...
const validarCorreoRegister = () => {
	if (
		correoRegisterInput.value.length === 0 ||
		correoRegisterInput.value === "" ||
		correoRegisterInput.value === null ||
		/^\s+$/.test(correoRegisterInput.value)
	) {
		Swal.fire({
			icon: "error",
			title: "Error...",
...
const validarContactoRegister = () => {
	if (
		contactoRegisterInput.value.length === 0 ||
		contactoRegisterInput.value === "" ||
		contactoRegisterInput.value === null ||
		/^\s+$/.test(contactoRegisterInput.value)
	) {
		Swal.fire({
			icon: "error",
			title: "Error...",
...
const validarContrasenaRegister = () => {
	if (
		contrasenaRegisterInput.value.length === 0 ||
		contrasenaRegisterInput.value === "" ||
		contrasenaRegisterInput.value === null ||
		/^\s+$/.test(contrasenaRegisterInput.value)
	) {
		Swal.fire({
			icon: "error",
			title: "Error...",
...
nombre_completoRegisterInput.addEventListener(
	"focusout",
